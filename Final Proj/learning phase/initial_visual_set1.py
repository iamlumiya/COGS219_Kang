...
subj_code = "initial_v_s1_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
...
trials = list(zip(stimuli_df['visual_s1'], stimuli_df['name_s1']))
...
separator = ','
data_file.write(separator.join(["subj_code", "image_path", "name_text"]) + '\n')
...
name_display = visual.TextStim(win, text ="", font ='Arial', color = 'white', height = 35, pos =(0,110))
...
for i in range (0, len(trials), 12):
    ...
    for image_path, name_text in block:
        ...
        trials.append([subj_code, image_path, name_text]) 
